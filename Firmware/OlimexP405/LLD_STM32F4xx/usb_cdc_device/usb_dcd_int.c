...
static uint32_t DCD_WriteEmptyTxFifo(USB_OTG_CORE_HANDLE *pdev, uint32_t epnum)
{
            ...
            ep->xfer_len != 0)
  {
    ...
    #if 1
		//This chunk of code will disable USB IRQ emptyintr
		//Problem: Initialization and deinitialization of USB VCP (COM open)
		//Will take several seconds. - Fix disabled fetching of `len` variable
    //which was used in FTDI systick event state machine to verify if data 
    //were sent or not yet. Removed and now everything works as it should.

		//Advantage: Firmware is not flooded with IRQs and can operate normally
		//with or without opened COM
