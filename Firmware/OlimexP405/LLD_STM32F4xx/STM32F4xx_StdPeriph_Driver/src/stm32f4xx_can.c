...
/**
  * @brief  Initiates and transmits a CAN frame message. Uses only mailbox 0
  * @param  CANx: where x can be 1 or 2 to to select the CAN peripheral.
  * @param  TxMessage: pointer to a structure which contains CAN Id, CAN DLC and CAN data.
  * @retval The number of the mailbox that is used for transmission or
  *         CAN_TxStatus_NoMailBox if there is no empty mailbox.
  */
uint8_t CAN_Transmit_Mailbox0(CAN_TypeDef* CANx, CanTxMsg* TxMessage)
{
  uint8_t transmit_mailbox = 0;

  //Check if mailbox 0 is ready. Do not use any other mailbox
	//Otherwise during tranmission of sequences we can get inconsistent rows
	//i.e. ISO15765 can have 1-2-4-3-5 CAN messages, which violates protocol and ECU
	//is going to ignore this packet. Using only one mailbox and transmitting
	//only when mailbox is empty will guarantee that we are going to transmit always
	//as 1-2-3-4-5-6...
  if ((CANx->TSR&CAN_TSR_TME0) == CAN_TSR_TME0)
  {
    transmit_mailbox = 0;
